...
# DEFINISI DAN SPESIFIKASI
# andBitInt : 2 int --> int
#    {andBitInt (a, b) adalah fungsi mengoperasikan AND bilangan bulat dengan mengubah dulu ke biner lalu hasilnya akan diuubah lagi ke bilangan bulat}

# def nbEqual (a, b) :                       # menyesuaikan jumlah bit pada biner yang lebih kecil
#     if nbElemen(biner(a)) > nbElemen(biner(b)) :
#         return konkat([0]*(nbElemen(biner(a))-nbElemen(biner(b))), biner(b)) 
#     elif nbElemen(biner(a)) < nbElemen(biner(b)) :
#         return konkat([0]*(nbElemen(biner(b))-nbElemen(biner(a))), biner(a)) 
    
def andBit (L1, L2) :                      # operasi AND bit dengan keluaran biner
    if isEmpty(L1) or isEmpty(L2) :
        return []
    else :
        return konsi(andBit(head(L1), head(L2)), lastElmt(L1)*lastElmt(L2))
    
def andBitInt (a, b) :
    return TranslateBinary(andBit(biner(a), (biner(b))))


print(biner(4))
print(andBit([1,0,0], [1,0,0]))
print(andBitInt(4, 4))
