...
# T dalam bentuk list
def SearchXChild (x, T) :
    if isTreeNEmpty(T) :
        return False
    else :
        return SearchXTree(x, FirstList(T)) or SearchXChild(x, TailList(T))
...
def SearchXTree (x, T) :
    if isTreeNEmpty(T) :
        ...
        if x == Akar(T) :
            ...
            return SearchXTree(x, FirstList(Anak(T))) or SearchXChild(x, TailList(Anak(T)))
